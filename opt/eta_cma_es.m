function [xmin,fmin,counteval,xmean,sigma,C,max_D,min_D] = eta_cma_es(fun, bbob, mu, lambda, xmean, sigma, STOP, PRINT_SHOW)
    ...
    ETA_MIN = 0.01;
    ETA_MAX = 1;
    eta_m = nan*ones(g_save,1); eta_m(1:wait) = 1;
    P_g = nan*ones(g_save,1);
    %eta_s = ones(g_save,1); % not implemented
    ...
    while counteval < STOP.FEVAL_MAX
        ...
        dm = xmean-xmean_old;
        % S_old = sigma_old^2*C_old; % S = sigma^2*C;
        % dS = S - S_old;

        %% APOP PERFORMANCE MEASURE 
        if INHERIT_SMALL == 1 
            ...
            if wait <= 0
                if P==P_Thr     % neutral performance
                    eta_m(g) = eta_m(g-1);
                elseif P<P_Thr  % positive performance
                    eta_m(g) = eta_m(g-1)*FACTOR_ETA;
                elseif P>P_Thr  % negative performance
                    eta_m(g) = eta_m(g-1)/FACTOR_ETA;
                end
                eta_m(g) = min(max(eta_m(g), ETA_MIN), ETA_MAX); % keep in bounds

                if eta_m(g) ~= eta_m(g-1)  
                    wait = L; % wait only if pop. change happened
                    sigma = sigma*(eta_m(g-1)/eta_m(g));  
                end
            ...
            %% UPDATE
            xmean = xmean_old + eta_m(g)*dm;
           
            if g<=L
                f_med_g(g) = median(arfitness(1:mu));
            else
                f_med_g = [f_med_g(2:end), median(arfitness(1:mu))];
            end
        end % Adapt learning rate

        %% update new function value
        % frec = fun(xmean); counteval=counteval+1; %fmin: alternatively
        ...
        if mod(g, g_save)==0 % preallocate g_save slots
            eta_m = [eta_m; nan*zeros(g_save,1)];
            P_g = [P_g; nan*zeros(g_save,1)];
        ...
        if PRINT_SHOW==1
            disp(['g:',num2str(g,'%i'), ' feval:', num2str(counteval,'%i'), ' fbest:' num2str(fmin), ' sig*D:' ... 
                  ...
                  num2str(max_D/min_D,'%.2e'), ' eta_m:', num2str(eta_m(g),'%.4e')]);
        end
        g = g+1;
    ...
    if PRINT_SHOW==1
        figure; subplot(1,2,1); hold on; title('eta-cma-es');
            plot(f_g, 'DisplayName','f');
            plot(sig_g, 'DisplayName','sigma');       
            %plot(sigD_g, 'DisplayName','sig*D');    
            plot(eta_m, 'DisplayName','eta m');
            %plot(eta_s, 'DisplayName','eta s');
            set(gca, 'YScale', 'log'); legend;
        subplot(1,2,2);
            plot(P_g, 'k', 'DisplayName','$P$');
            yline(P_Thr, 'r--', 'DisplayName','$P_{Thr}$')
            ylim([0,1]); legend; box off;
    end
